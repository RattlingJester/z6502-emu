...
test "nested JSR/RTS" {
    ...
    _ = try cpu.step(); // JSR $9000
    _ = try cpu.step(); // JSR $A000
    _ = try cpu.step(); // RTS -> back to 0x9003
    try testing.expectEqual(@as(u16, 0x9003), cpu.PC);
    _ = try cpu.step(); // RTS -> back to 0x8003
...
test "PHA_IMPL / PLA_IMPL round-trip" {
    ...
    cpu.A = 0x42;
    const push_cycles = try cpu.step();
    cpu.A = 0x00; // clobber to prove PLA restores it
    const pop_cycles = try cpu.step();
    try testing.expectEqual(@as(u8, 0x42), cpu.A);
